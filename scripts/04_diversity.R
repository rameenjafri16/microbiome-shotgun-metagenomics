...
# 5. HEATMAP — TOP 30 SPECIES -----------------
# Shows per-sample species-level abundance, revealing S. copri enterotype
# clustering and inter-individual variation across both dietary groups

library(pheatmap)

# Get top 30 most abundant species by mean relative abundance
otu_rel_mat <- sweep(as.matrix(otu_counts), 2, colSums(otu_counts), "/")
top30 <- names(sort(rowMeans(otu_rel_mat), decreasing = TRUE))[1:30]
heatmap_mat <- otu_rel_mat[top30, ]

# Clean up column names to readable sample labels
colnames(heatmap_mat) <- c("Vegan_1", "Vegan_2", "Vegan_3",
                           "Omnivore_1", "Omnivore_2", "Omnivore_3")

# Log-transform for better colour scaling (add small pseudocount)
heatmap_log <- log10(heatmap_mat + 1e-5)

# Annotation bar showing diet group
annotation_col <- data.frame(
  Diet = c("Vegan", "Vegan", "Vegan", "Omnivore", "Omnivore", "Omnivore"),
  row.names = colnames(heatmap_log)
)
annotation_colors <- list(
  Diet = c(Vegan = "#2ecc71", Omnivore = "#e74c3c")
)
...
pheatmap(
  ...
  main              = "Top 30 Species by Mean Abundance (log10 relative abundance)",
  filename          = "figures/heatmap_top30.png",
  width             = 10,
  height            = 9
)
cat("Saved: figures/heatmap_top30.png\n")
